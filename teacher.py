...
with open('/content/drive/MyDrive/FittingAuditory/Freqz/GAM.pkl', 'rb') as fp:
    GAM = pickle.load(fp)
with open('/content/drive/MyDrive/FittingAuditory/Freqz/VQT.pkl', 'rb') as fp:
    VQT = pickle.load(fp)
with open('/content/drive/MyDrive/FittingAuditory/Freqz/THIRD.pkl', 'rb') as fp:
...
class SpectrogramDataModule(pl.LightningDataModule):
    def __init__(self, sav_dir, domain, batch_size):
        ...
        if domain == "speech":
            self.feature = "mel"
            self.coef_dir = '/content/drive/MyDrive/FittingAuditory/Freqz/GAM.pkl'
            self.data_dir = '/content/drive/MyDrive/FittingAuditory/audio/NTVOW/'
        elif domain == "music":
            self.feature = "vqt"
            self.coef_dir = '/content/drive/MyDrive/FittingAuditory/Freqz/VQT.pkl'
            self.data_dir = "/content/drive/MyDrive/FittingAuditory/audio/SOL/"
        elif domain == "urban":
            self.feature = "third_oct_response"
            self.coef_dir = '/content/drive/MyDrive/FittingAuditory/Freqz/THIRD.pkl'
            self.data_dir = "/content/drive/MyDrive/FittingAuditory/audio/SONYC-UST-dev/"
    ...
    def collate_batch(self, batch):
        ...
        feat = torch.stack([torch.tensor(s['feature']) for s in batch], axis=0) # Convert 'feature' to tensor
        x = torch.stack([torch.tensor(s['x']) for s in batch]) # Convert 'x' to tensor
        return {'feature': feat, 'x': x}
